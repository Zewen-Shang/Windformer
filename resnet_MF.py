...
class ResNet_Fig_MF(Module):
    def __init__(self,feature_num,window_size,img_shape,depth,drop_prob) -> None:
        super().__init__()
        ...
        self.batchnorm0 = nn.BatchNorm2d(feature_num * window_size)
        self.relu0 = nn.ReLU()
        self.conv0 = nn.Conv2d(feature_num * window_size,64,5,padding="same")
        self.maxpool0 = MaxPool2d(2,2)

        self.resblocks = nn.ModuleList([ResBlock(64,64,drop_prob) for i in range(self.depth)])

        self.linear = Linear(img_shape[0]//2 * img_shape[1] // 2 * 64,img_shape[0] * img_shape[1])

    def forward(self,input):
        batch_size = input.shape[0]
        # input (batch_size,feature_num,window_size,30,20)
        # x (batch_size,feature_num * window_size,30,20)
        x = input.reshape(batch_size,self.feature_num * self.window_size,self.img_shape[0],self.img_shape[1])
        ...
        x = x.reshape(batch_size,self.img_shape[0] // 2 * self.img_shape[1] // 2 * 64)
        x = self.linear(x)
