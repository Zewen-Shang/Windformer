...
class CNN(nn.Module):
    ...
    def forward(self,input):
        ...
        for block in self.blocks:
            x = block(x)
        x = self.conv1(x)
        x = x.reshape(batch_size,self.window_size,self.feature_num,self.img_shape[0] * self.img_shape[1])
        x = x.permute(0,1,3,2)
        return x
...
class CNN_Trans(nn.Module):
    def __init__(self,feature_num,img_shape,window_size,hidden_channels,depth,hidden_size,temporal_dim,temporal_head) -> None:
        ...
        self.cnn = CNN(window_size,feature_num,img_shape,hidden_channels=hidden_channels,depth=depth)
