...
class DualAttn(nn.Module):
    def __init__(self,drive_num,window_size,hidden_size,target_pos) -> None:
        super().__init__()
        self.drive_num = drive_num
        self.window_size = window_size
        self.hidden_size = hidden_size
        ...
        self.encoder = Encoder(drive_num=drive_num,window_size=window_size,hidden_size=hidden_size)
        self.decoder = Decoder(drive_num=drive_num,window_size=window_size,hidden_size=hidden_size)
    ...
    def forward(self,input):
        ...
        y = input[:,:,self.target_pos[0],self.target_pos[1]]
        x = input.reshape(batch_size,self.window_size,self.drive_num).permute(0,2,1)
