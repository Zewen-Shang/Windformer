...
class Vit_Seq(nn.Module):
    def __init__(self,feature_num,window_size,drive_num,dim,mlp_dim,depth,num_heads,drop_prob) -> None:
        super().__init__()
        self.feature_num,self.window_size,self.drive_num = feature_num,window_size,drive_num
        self.dim,self.mlp_dim,self.depth = dim,mlp_dim,depth
        self.num_heads,self.drop_prob = num_heads,drop_prob

        self.pos_embed = nn.Parameter(torch.randn(1,drive_num,dim))
        self.cls_token = nn.Parameter(torch.rand(1,1,dim))

        self.feature_embed = nn.Linear(feature_num * window_size,dim)
    ...
    def forward(self,input):
        ...
        x = input.reshape(batch_size,self.feature_num * self.window_size,self.drive_num).permute(0,2,1)
        ...
        x = x.view(batch_size,self.drive_num+1,self.dim)
        x = x[:,0]
